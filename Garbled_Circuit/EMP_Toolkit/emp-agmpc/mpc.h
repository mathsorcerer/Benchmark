...
class CMPC { public:
	...
	CMPC(NetIOMP<nP> * io[2], ThreadPool * pool, int party, BristolFormat * cf, int * num_and_gates, 
		block (*GTM)[4][nP+1], block (*GTK)[4][nP+1], bool (*GTv)[4], block (*GT)[nP+1][4][nP+1], 
		block * labels, bool* value, block * eval_labels[nP+1], bool * _delta = nullptr, int ssp = 40) {
		...
		for(int i = 1; i <= nP; ++i)
			this->eval_labels[i] = eval_labels[i];
...
//		ret.get();
	...
	void online(uint8_t *input, uint8_t *output, int num_ands) 
	{
		...
		if(party != 1) 
		{
			// zmq::message_t mip(num_in);
        	// memcpy((void*)mip.data(), mask_input, num_in);
        	// socket_req->send(mip); 
			
			// zmq::message_t reply;
        	// socket_req->recv(&reply);
			// memcpy(mask_input, reply.data(), num_in);
		...
		else 
		{
			...
			for(int i = 2; i <= nP; ++i) 
			{
				int party2 = i;
				// zmq::message_t reply;
				// socket_rep->recv(&reply);
				// memcpy(tmp[party2], reply.data(), num_in);
		...
		if(party != 1) 
		{
			...
			// zmq::message_t mblk(num_in*sizeof(block));
        	// memcpy((void*)mblk.data(), tmp, num_in*sizeof(block));
        	// socket_req->send(mblk); 
		...
		else 
		{
			for(int i = 2; i <= nP; ++i) 
			{
				...
				// zmq::message_t reply;
				// socket_rep->recv(&reply);
				// memcpy(eval_labels[party2], reply.data(), num_in*sizeof(block));

				// string last_msg = "Done";
				// zmq::message_t lmsg(last_msg.length());
				// memcpy((void*)lmsg.data(), last_msg.c_str(), last_msg.length());
				// socket_rep->send(lmsg); 
			...
			for(int i = 0; i < num_gates; i+=4) 
			{
				...
				{
					case XOR_GATE:
						mask_input[g2] = mask_input[g0] != mask_input[g1];
						for(int j = 2; j <= nP; ++j)
							eval_labels[j][g2] = eval_labels[j][g0] ^ eval_labels[j][g1];		
						break;
					case AND_GATE:
						index = (mask_input[g0] << 1) + mask_input[g1];
						
						for(int j = 2; j <= nP; ++j)
							eval_labels[j][g2] = GTM[ands][index][j];
						mask_input[g2] = GTv[ands][index];

						for(int j = 2; j <= nP; ++j) 
						{
							Hash(H, eval_labels[j][g0], eval_labels[j][g1], ands, index);
							xorBlocks_arr(H, H, GT[ands][j][index], nP+1);

							for(int k = 2; k <= nP; ++k)
								eval_labels[k][g2] = H[k] ^ eval_labels[k][g2];

							t0 = GTK[ands][index][j] ^ Delta;
							
							if(cmpBlock(&H[1], &GTK[ands][index][j], 1))
								mask_input[g2] = mask_input[g2] != false;
							else if(cmpBlock(&H[1], &t0, 1))
								mask_input[g2] = mask_input[g2] != true;
							else { cout << ands << "no match GT!" << endl << flush; } 
						}
						ands++;
						break;
					default:
						mask_input[g2] = not mask_input[g0];
						for(int j = 2; j <= nP; ++j)
							eval_labels[j][g2] = eval_labels[j][g0];
						break;
				}
			}
