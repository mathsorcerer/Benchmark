...
class ABitMP { public:
	...
	future<void> check(block * MAC[nP+1], block * KEY[nP+1], bool* data, int64_t length) {
	...
	void check1(block * MAC[nP+1], block * KEY[nP+1], bool* data, int64_t length) {
		...
		for(int64_t i = 0; i < ssp*length; ++i)
			tmp[i] = tmp[i] % 4;
//		for(int j = 0; j < ssp; ++j) {
...
//		}
		...
		for(int64_t i = 0; i < length; i+=chk) {
			tb[i/chk][1] = data[i];
			tb[i/chk][2] = data[i+1];
		...
		for(int k = 1; k <= nP; ++k) if(k != party) {
			uint8_t * tmpptr = tmp;
			for(int64_t tt = 0; tt < length/SIZE; tt++) {
				int start = SIZE*tt;
				for(int64_t i = SIZE*tt; i < SIZE*(tt+1) and i < length; i+=chk) {
	...
	void check2(block * MAC[nP+1], block * KEY[nP+1], bool* data, int64_t length) {
