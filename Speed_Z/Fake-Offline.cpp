...
void make_dabits(const typename T::mac_type& key, int N, int ntrip, bool zero,
    const typename T::bit_type::mac_type& bit_key = { })
{
  printf("***** Calling make_dabits *****\n");
  auto start_make_dabits = clock_start();
  ...
  for (int i = 0; i < ntrip; i++)
    ...
    printf("Time to make dabits with %d parties for %d bits: %f (us)\n", N, ntrip, time_from(start_make_dabits));
...
void FakeParams::make_edabits(const typename T::mac_type& key, int N, int ntrip, bool zero, false_type,
    const typename T::bit_type::mac_type& bit_key)
{
  printf("***** Calling make_edabits *****\n");
  auto start_make_edabits = clock_start();
  ...
  for (auto length : lengths)
    ...
    printf("Time to make edabits with %d parties for %d bits: %f (us)\n", nplayers, ntrip, time_from(start_make_edabits));
